...
func (s *Store) SetMistralKey(ctx context.Context, userID string, key []byte) error {
	_, err := s.pool.Exec(ctx, `
		UPDATE users SET mistral_api_key_enc = $2, updated_at = NOW() WHERE id = $1
	`, userID, key)
	return err
}

func (s *Store) GetMistralKey(ctx context.Context, userID string) ([]byte, error) {
	var key []byte
	err := s.pool.QueryRow(ctx, `
		SELECT mistral_api_key_enc FROM users WHERE id = $1
	`, userID).Scan(&key)
	return key, err
}
