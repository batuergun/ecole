...
func (s *Store) CreateTrainingRun(ctx context.Context, projectID, baseModel, computeMode string, hfFlavor, hfNamespace *string, loraConfig, trainingConfig json.RawMessage) (*model.TrainingRun, error) {
	...
	err := s.pool.QueryRow(ctx, `
		...
		RETURNING id, project_id, base_model, lora_config, training_config, compute_mode, hf_flavor, hf_namespace,
			...
			output_model_path, hf_repo_id, trackio_url, error_message, started_at, completed_at, created_at, updated_at
	`, projectID, baseModel, loraConfig, trainingConfig, computeMode, hfFlavor, hfNamespace).Scan(
		...
		&t.OutputModelPath, &t.HFRepoID, &t.TrackioURL, &t.ErrorMessage, &t.StartedAt, &t.CompletedAt, &t.CreatedAt, &t.UpdatedAt,
	)
	if err != nil {
		return nil, err
	}
...
func (s *Store) SetTrackioURL(ctx context.Context, id, url string) error {
	_, err := s.pool.Exec(ctx, `UPDATE training_runs SET trackio_url = $2, updated_at = NOW() WHERE id = $1`, id, url)
	return err
}
